...
public:           
	...
	bool addNode(NodeType data, int index, sf::Vector2f pos);
	...
	void UCSFindHeuristics(Node* pStart, Node* pDest);
	void ResetData(Node& node);
	void UnmarkNodes(Node* pCurrent);
	void setValueToInfinate(Node* pNode);

	float heurPercent = .90f;
	int const m_STEPCOST = 1;
	void AStarPathfinding(Node* pStart, Node* pTarget, vector<Node *> & path);
	void CalculateHeuristic(Node * node, Node * dest);
...
// ----------------------------------------------------------------
template<class NodeType, class ArcType>
bool Graph<NodeType, ArcType>::addNode( NodeType data, int index , sf::Vector2f pos) {
...
void Graph<NodeType, ArcType>::UnmarkNodes(Node* pCurrent)
{
	...
	while (!pq.empty())
	{
		...
		for (; iter != endIter; iter++)
		{
			...
			(*iter).node()->setPreviousNode(NULL);
...
{
	pNode->setData(pair<string, int>(pNode->getData().first, (INT_MAX - 1000)));
	pNode->SetF(INT_MAX - 1000);
}
...
void Graph<NodeType, ArcType>::AStarPathfinding(Node* pStart, Node* pTarget, vector<Node *> & path)
{
	...
	while (!pq.empty() && pq.top() != pTarget)
	{
		...
		for (; iter != endIter; iter++)
		{
			if ((*iter).node() != pq.top()->getPreviousNode())
			{
				int hn = pq.top()->getArc((*iter).node())->weight() + pq.top()->getData().second;
	...
	while (pTarget->getPreviousNode() != NULL)
	{
		...
		pTarget = pTarget->getPreviousNode();
	}
	path.push_back(pTarget);
}

template<class NodeType, class ArcType>
void Graph<NodeType, ArcType>::CalculateHeuristic(Node * node, Node * dest)
{
	sf::Vector2f v1 = sf::Vector2f(node->GetShape().getPosition().x, node->GetShape().getPosition().y);
	sf::Vector2f v2 = sf::Vector2f(dest->GetShape().getPosition().x, dest->GetShape().getPosition().y);

	node->SetH(sqrt(pow(v2.x - v1.x, 2) + pow(v2.y - v1.y, 2)));
}
